...
Running the three built-in cases exercises the full range of decision outcomes the system can produce:

| Case | Decision | Typical Risk Score | What it demonstrates |
|------|----------|--------------------|----------------------|
| **TC-001** | ✅ **APPROVED** | Low (~20s) | Clean approval — strong credit, low DTI, ample reserves, no compliance flags |
| **TC-002** | ⚠️ **CONDITIONAL_APPROVAL** | Moderate (~40s–50s) | Curable issues — self-employed income and documentation conditions trigger human review |
| **TC-003** | ❌ **DENIED** | High (~70s+) | Clear denial — low credit, high DTI, and disqualifying credit events |

> Exact risk scores vary slightly between runs because each analysis is generated by an LLM, but the decision outcomes are stable across the three borrower profiles.
